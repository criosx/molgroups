...
d_oxide = Parameter(name='silicon oxide layer', value=10).range(5, 30)
d_Cr =  Parameter(name='chromium layer', value=40).range(10, 150)
d_gold =  Parameter(name='gold layer', value=100).range(150, 250) #thickness of gold
...
mult_tether =  Parameter(name='multiplicity tether', value=2).range(0.1, 4) #ratio of bME to tether molecules at surface
l_tether =  Parameter(name='length tether', value=10).range(3, 18) #distance from substrate to inner headgroup/acyl chain interface
...
layer_gold = Slab(material=gold, thickness=d_gold - (blm.substrate.z + 0.5 * blm.substrate.l), interface=0.0000) #thickness can also be 20 angstroms
