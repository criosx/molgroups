...
class CFitInteractor():
    ...
    def fnLoadSingleColumns(self, sFileName, data, exceptions=(), header=True, headerline=(), LoadList=()):
        ...
        for i,columname in enumerate(splitheaderline):
        ...
        for line in content:
            ...
            for i,column in enumerate(splitline):
                if LoadList==[] or (splitheaderline[i] in LoadList):
                    data[splitheaderline[column]].append(float(splitline[column]))

        return data



    def fnLoadSingleRows(self, sFileName, data, exceptions=()):
        """
    ...
    def fnSaveSingleColumns(self, sFilename, diData):
        """
        Saves all data out to a file using columns. diData is a dictionary and the key values
        are used for the header.
        """

        file=open(sFilename,"w")
        file.writelines([" ".join([element for element in diData])+'\n'])              #write out headers
        file.writelines([" ".join([str(diData[element][i]) for element in diData])+'\n'
                         for i in range(len(diData[diData.keys()[0]]))])
        file.close()
...
#-------------------------------------------------------------------------------
    ...
    def fnAnalyzeStatFile(self, fConfidence):                                   #summarizes stat file
        ...
        self.diStatResults= {
            'Parameters': {}}                                                   #Prepare dictionary for analysis results
                                                                                #related entries
...
#-------------------------------------------------------------------------------
    ...
    def fnCalculateMolgroupProperty(self,sMolgroup,fConfidence):
        ...
        diResults= {'Center of Mass': [], 'Total Integral': [], 'Average': []}
...
#-------------------------------------------------------------------------------
    ...
    def fnLoadStatResult(self, filename):                                          #Load a file like iSErr or SErr into
                                                                                #the self.liStatResult object
        self.sStatResultHeader=''
        self.liStatResult=[]
        try:
            file = open(filename)                                               #open and read file
...
#-------------------------------------------------------------------------------
    ...
    def fnPullMolgroup(self, statfilename, liMolgroupNames):
        ...
        """
        self.fnRecreateStatistical(statfilename)
        ...
        self.cInteractor.fnSaveSingleColumns('pulledmolgroups.dat',di)
        self.cInteractor.fnSaveSingleColumns('pulledmolgroupsstat.dat',diStat)
...
def fnDetermineFitSoftware():
    ...
    if os.path.isfile('setup.c') or os.path.isfile('setup.cc'):
...
if __name__ == '__main__':
    if len(sys.argv)==1:
        ...
        cInteractor=CGaReflInteractor() if fnDetermineFitSoftware()=='garefl' else CRefl1dInteractor()
        ReflPar=CReflPar(cInteractor)
        ...
        for i in range(1,len(sys.argv)):
            ...
            if sys.argv[i] == '-a' :
                ...
                ReflPar.fnPullMolgroup(fnDetermineStatFile(),sys.argv[i+1:])
            elif sys.argv[i] == '-r' :
                Result()
