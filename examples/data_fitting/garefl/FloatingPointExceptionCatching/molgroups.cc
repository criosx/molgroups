...
void tBLM_quaternary_chol::fnAdjustParameters(){
    ...
    defect_headgroup->nSL=defectratio*(headgroup2->nSL*headgroup2->nf+headgroup2_2->nSL*headgroup2_2->nf+headgroup2_3->nSL*headgroup2_3->nf);
...
{
    fprintf(fp, "Discrete %s StartPosition %e Beta %g Gamma %g nf %g \n",cName, dStartPosition,dBeta, dGamma, nf);
    nSLDObj::fnWriteData2File(fp, cName, dimension, stepsize);    
}
...
double Hermite::fnGetArea(double dz) {
    double p0, p1, m0, m1, km1, k0, k1,h00, h01, h10, h11, t, dd, t_2, t_3, volfrac, tau;
    double alpha, beta;
    int interval;  
    ...
    if (interval>=0) {
        ...
        if (monotonic==1) {                                      //Monotonic cubic spline
                                                  
            if (vf[interval]==vf[interval+1]) {
                m0=0;
                m1=0;
            }
            else {
                k0=(vf[interval+1]-vf[interval])/(dp[interval+1]-dp[interval]);
                if (interval==0) {
                    km1=k0;
                    k1=(vf[interval+2]-vf[interval+1])/(dp[interval+2]-dp[interval+1]);
                }
                else if (interval==(numberofcontrolpoints-2)) {
                    km1=(vf[interval]-vf[interval-1])/(dp[interval]-dp[interval-1]);
                    k1=k0;
                }
                else {
                    km1=(vf[interval]-vf[interval-1])/(dp[interval]-dp[interval-1]);
...
double SLDHermite::fnGetnSLD(double dz) {
    double p0, p1, m0, m1, km1, k0, k1,h00, h01, h10, h11, t, dd, t_2, t_3, scattlength, tau;
    double alpha, beta;
    int interval;
    ...
    if (interval>=0) {
        ...
        if (monotonic==1) {                                      //Monotonic cubic spline
            ...
            if (sld[interval]==sld[interval+1]) {
                m0=0;
                m1=0;
            }
            else {
                k0=(sld[interval+1]-sld[interval])/(dp[interval+1]-dp[interval]);
                if (interval==0) {
                    km1=k0;
                    k1=(sld[interval+2]-sld[interval+1])/(dp[interval+2]-dp[interval+1]);
                }
                else if (interval==(numberofcontrolpoints-2)) {
                    km1=(sld[interval]-sld[interval-1])/(dp[interval]-dp[interval-1]);
                    k1=k0;
                }
                else {
                    km1=(sld[interval]-sld[interval-1])/(dp[interval]-dp[interval-1]);
            ...
            if (interval==0) {
                m0=0;
                m1=(sld[2]-sld[0])/(dp[2]-dp[0]);
            }
            else if (interval==(numberofcontrolpoints-2)) {
                m0=(sld[interval+1]-sld[interval-1])/(dp[interval+1]-dp[interval-1]);
                m1=0;
            }
            else {
                m0=(sld[interval+1]-sld[interval-1])/(dp[interval+1]-dp[interval-1]);
                m1=(sld[interval+2]-sld[interval])/(dp[interval+2]-dp[interval]);
            }
        }
        ...
        scattlength=h00*p0+h10*dd*m0+h01*p1+h11*dd*m1;
        //printf("m0 %e m1 %e p0 %e p1 %e dd %e dz %e t %e normarea %e", m0, m1, p0, p1, dd, dz, t, normarea);
