...
def bilayer(z, sigma, bulknsld, global_rough, rho_substrate, l_surfasil, n_surfasil):
    ...
    substrate.fnSet(nSL=rho_substrate*substrate.vol, sigma=global_rough)
    surfasil.fnSet(length=l_surfasil, position=20+0.5*l_surfasil, nf=n_surfasil)
    ...
    normarea, area, nsl = surfasil.fnOverlayProfile(z, area, nsl, normarea)
...
n_surfasil = Parameter(name='surfasil roughness', value = 5.).range(2, 20)
...
substrate = mol.Box2Err(dz=10, dsigma1=0, dsigma2=2, dlength=20, dvolume=200, dnSL=0, dnumberfraction=1, name='siox')
surfasil = mol.Box2Err(dz=50, dsigma1=2, dsigma2=2, dlength=60, dvolume=10, dnSL=0, dnumberfraction=1, name='surfasil')
...
mollayer = FunctionalProfile(dimension*stepsize, 0, profile=bilayer, sigma=sigma, bulknsld=d2o.rho,
                             global_rough=global_rough, rho_substrate=siox.rho)
...
# === Data files ===
probe = load4('../ssBLM/ch061.refl', back_reflectivity=True)
probeh = load4('../ssBLM/ch060.refl', back_reflectivity=True)
