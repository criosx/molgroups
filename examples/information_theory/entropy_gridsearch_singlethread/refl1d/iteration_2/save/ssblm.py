...
vf_bilayer = Parameter(name='volume_fraction_bilayer', value= 0.5).range( 0.0, 1.0)
...
###################################################################
layer_siox.interface = siox_interface = Parameter(name='siox_interface', value= 5.5).range( 2.0, 9.0)

# Si and SiOx roughnesses are the same
layer_silicon.interface = layer_siox.interface

# nSLD parameters
d2o.rho = rho_d2o = Parameter(name='rho_solv_0', value= 5.9).range( 5.3, 6.5)
h2o.rho = rho_h2o = Parameter(name='rho_solv_1', value= 0.0).range( -0.6, 0.6)
tiox.rho = rho_tiox = Parameter(name='rho_tiox', value= 0.8417100000000001).range( -0.15828999999999982, 1.84171)
siox.rho = rho_siox = Parameter(name='rho_siox', value= 4.1).range( 3.1, 5.1)

# layer thickness parameters
layer_tiox.thickness = tiox_thickness = Parameter(name='tiox_thickness', value= 166.3795).range( 66.379, 266.38)
layer_siox.thickness = siox_thickness = Parameter(name='siox_thickness', value= 22.5).range( 5.0, 40.0)
...
probe.background = background_0 = Parameter(name='background_0', value= 4.95e-06).range( -1e-07, 1e-05)
probeh.background = background_1 = Parameter(name='background_1', value= 4.95e-06).range( -1e-07, 1e-05)
probe.intensity = intensity = Parameter(name='intensity', value= 0.9750000000000001).range( 0.9, 1.05)
probeh.intensity = probe.intensity
probe.theta_offset = theta_offset = Parameter(name='theta_offset', value= -0.004999999999999999).range( -0.015, 0.005)
probeh.theta_offset = probe.theta_offset
probe.sample_broadening = sample_broadening = Parameter(name='sample_broadening', value= 0.0075).range( -0.005, 0.02)
