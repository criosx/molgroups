...
# Modified 15-Sep-2011 F.H.

import math, os, random, re, sys, string, subprocess, time, operator
import scipy, scipy.stats, scipy.special, numpy
...
#-------------------------------------------------------------------------------
    ...
    def fnAnalyzeStatFile(self, fConfidence):                                   #summarizes stat file
        ...
        try:
            ...
            self.fnLoadStatResult(sFileName)                                    #Load data from file into list
            self.fnLoadParameters()                                             #Load Parameters for limits

        except:
            print 'Was not able to load Stat file for analysis.'

        self.diStatResults={}                                                   #Prepare dictionary for analysis results
        self.diStatResults['Parameters']={}                                     #set up dictionary containing parameter
                                                                                #related entries
        ...
        for i in range(len(self.sStatResultHeader.split())):                    #cycle through all parameters
            sParameterName=self.sStatResultHeader.split()[i]
            if len(sParameterName)>iMaxParameterNameLength:                     #determine length of longest parameter name for displaying
                iMaxParameterNameLength=len(sParameterName)
            li=[]
            for j in range(len(self.liStatResult)):                             #cycle through all MC iterations
                li.append(float(self.liStatResult[j].split()[i]))               #and fetch the values of the parameter
            self.diStatResults['Parameters'][sParameterName]={}
            self.diStatResults['Parameters'][sParameterName]['Values']=li[:]    #save all data
        self.diStatResults['MaxParameterLength']=iMaxParameterNameLength


        iNumberOfMCIterations=len(self.liStatResult)                            #how many iterations already done
        print 'Analysis of current MC simulation ...'
        print 'Number of iterations: %(ni)d' % {'ni':iNumberOfMCIterations}
        self.diStatResults['NumberOfIterations']=iNumberOfMCIterations
        ...
        for element in sorted(self.diParameters.keys(),key= lambda sParameter: self.diParameters[sParameter]['number']):
            ...
            for i in range(11):
                ...
                sGraphOutput=sGraphOutput+s1
            sGraphOutput=sGraphOutput+']'
            ...
            sPrintString='%(el)'
            sPrintString=sPrintString+str(iMaxParameterNameLength)
            sPrintString=sPrintString+'s  %(sg)s  [%(ll)10.4g,%(ul)10.4g]  [%(lp)10.4g(%(lpc).3f), %(m)10.4g(%(mc).3f), %(hp)10.4g(%(hpc).3f)] (-%(ld)10.4g, +%(hd)10.4g)'
...
#-------------------------------------------------------------------------------
    ...
    def fnBackup(self):
        ...
        pr.wait()
        pr=subprocess.call("cp fit*.dat rsbackup/", shell=True)
        pr=subprocess.Popen(["cp", "fit", "rsbackup/"])
        pr.wait()
        pr=subprocess.call("cp model*.dat rsbackup/", shell=True)
        pr=subprocess.Popen(["cp", "pop_bak.dat", "rsbackup/"])
        pr.wait()
        pr=subprocess.call("cp profile*.dat rsbackup/", shell=True)
        pr=subprocess.call("cp "+self.setupfilename+" rsbackup/", shell=True)
        pr=subprocess.Popen(["cp", "setup.o", "rsbackup/"])
        pr.wait()

#-------------------------------------------------------------------------------

    def fnCalculateMolgroupProperty(self,sMolgroup,fConfidence):
        ...
        try:
            ...
            print 'Failure to calculate values'


        diResults={}
        diResults['Center of Mass']=[]
        diResults['Total Integral']=[]
        diResults['Average']=[]

        for i in range(len(self.diStatResults['Molgroups'])):                                       #cycle over all MC iterations
                                                                                                    #Calculate Center of Mass and Integral

            fCOM,fInt=numpy.average(self.diStatResults['Molgroups'][i][sMolgroup]['zaxis'],
                                weights=self.diStatResults['Molgroups'][i][sMolgroup]['areaaxis'],
                                returned=True)
                                                                                                    #total integral in volume per surface area (unit: Å)
                                                                                                    #taking into account grid spacing of z-axis
            fInt=fInt/self.diStatResults['Molgroups'][i]['normarea']['areaaxis'][0]* \
            (self.diStatResults['Molgroups'][i]['normarea']['zaxis'][1]-
            self.diStatResults['Molgroups'][i]['normarea']['zaxis'][0])

            for j in range(len(self.diStatResults['Molgroups'][i][sMolgroup]['areaaxis'])):
                if self.diStatResults['Molgroups'][i][sMolgroup]['areaaxis'][j] != 0:
                    fCOM=fCOM-self.diStatResults['Molgroups'][i][sMolgroup]['zaxis'][j]                        #normalize COM to start of molecular group
                    break

            fAvg=numpy.average(self.diStatResults['Molgroups'][i][sMolgroup]['areaaxis'])

            diResults['Center of Mass'].append(fCOM)
            diResults['Total Integral'].append(fInt)
            diResults['Average'].append(fAvg)
        ...
        for element in diResults:
            ...
            sPrintString='%(el)s'
            sPrintString=sPrintString+'  [%(lp)10.4g, %(m)10.4g, %(hp)10.4g] (-%(ld)10.4g, +%(hd)10.4g)'

            print  sPrintString % \
            {'el':element,'lp': fLowPerc,'ld': (fMedian-fLowPerc), 'm': fMedian,'hd': (fHighPerc-fMedian), 'hp':fHighPerc}

        File=open("CalculationResults.dat","w")
        File.write("CenterOfMass TotalIntegral \n")
        for i in range(len(diResults['Center of Mass'])):
            File.write(str(diResults['Center of Mass'][i])+" ")
            File.write(str(diResults['Total Integral'][i])+"\n")
        File.close()
...
#-------------------------------------------------------------------------------
    ...
    def fnCheckFit(self):                                                       #checks fit directory for certain conditions
        ...
        try:
            self.fnLoadStatResult(self.sStatFileName)                               #Load data from file into list
            self.fnLoadParameters()                                                 #Load Parameters for limits
        except:
            pass

        if (self.sStatFileName!='') and (sorted(self.diParameters.keys())!=sorted(self.sStatResultHeader.split(None))):
            print '----------------------------------------------------'
            print '----------------------------------------------------'
...
#-------------------------------------------------------------------------------
    ...
    def fnContourData(self,sname,dZGrid,dRhoGrid):
        ...
        def fnMap2Array(liArray,liDimensions,dx1,dy1,dx2,dy2):                  #map nSLD profile onto array
            ...
            if diff==0:                                                         #treat single point differently because
                ...
                while i<diff:                                                   #and thus create surface/contour plot
                    ...
                    liArray[iY][iX]=liArray[iY][iX]+1
                    i=i+1



        self.fnRecreateStatistical(sname)                                       #Load Parameters and profiles from stat data
        ...
        while i<iNumberOfModels:                                                #loop through all models
            ...
            sFileName='ContArray'+str(i)+'.dat'                                 #write out array
            ...
            sFileName='ContDimZ'+str(i)+'.dat'                                  #dimension wave has one point extra for Igor
            file = open(sFileName,"w")
            ...
            sFileName='ContDimRho'+str(i)+'.dat'                                #dimension wave has one point extra for Igor
            file = open(sFileName,"w")
...
#-------------------------------------------------------------------------------
    ...
    def fnLoadParameters(self):                                                 #loads the last row's parameters, and
        ...
        self.chisq=float(string.split(data[-1])[1])                             #chi squared from second column last row
                                                                                ...
                                                                                #start searching setup file for molecular groups
                                                                                #first get dimension and stepsize of grid
#-------------------------------------------------------------------------------

    def fnLoadStatResult(self, sname):                                          #Load a file like iSErr or SErr into
                                                                                #the self.liStatResult object
        self.sStatResultHeader=''
        self.liStatResult=[]
        try:
            file = open(sname)                                                  #open and read file
            self.liStatResult=file.readlines()
            if self.liStatResult[0].split()[0]=='Chisq':                        #check if an iSErr has been opened
                print 'Reading isErr.dat ...'
                newdata=[]
                for line in self.liStatResult:                                  #chop off the first Chisq entry in each line
                    line=string.join(line.split(None)[1:]," ")
                    newdata.append(line)
                self.liStatResult=newdata
            else:
                print 'Reading sErr.dat ...'
            self.sStatResultHeader=self.liStatResult[0]                         #save header line separately
            self.liStatResult=self.liStatResult[1:]                             #delete header line
        except:
            raise 'Could not load statistical data!'
#-------------------------------------------------------------------------------
...
#-------------------------------------------------------------------------------
    ...
    def fnPlotMolgroups(self, plotname):
        ...
        for element in self.diMolgroups:
            nSLDtVolFrac=[]                                                     #calculate nSLD times volfrac
            if element!='normarea':                                             #Get normarea for nSLD calculations
        ...
        for parameter in litest:
            ...
            for i in range(10):                                                 #creates the par range ascii overview
                if ((fValue >= float(i)/10*fRange+fLowLim) and
                    ...
                    sRangeIndicator=sRangeIndicator+'|'
                else:
...
#-------------------------------------------------------------------------------
    def fnRecreateStatistical(self,sname,bRecreateMolgroups=True):              #Recreates profile and fit data
                                                                                #associated with stat file

        self.fnLoadParameters()                                                 #Load Parameters
        self.fnLoadStatResult(sname)                                            #Load Results from statistical analysis
        iNumberOfModels=self.fnGetNumberOfModelsFromSetupC()                    #how many profiles to analyze?
        j=0
        ...
        for statline in self.liStatResult:                                      #cycle through all individual stat. results
            try:
                self.fnBackup()                                                 #Backup setup.c, and other files
                self.diStatResults['nSLDProfiles'].append([])                   #appends a new list for profiles for the
                                                                                #current MC iteration
                splitstatline=statline.split()                                  #split stat. results into individual param.
                liParameters=self.diParameters.keys()                           #get list of parameters from setup.c/par.dat
                liParameters.sort(self.fndiParametersNumberSort)                #sort by number of appereance in setup.c
                if len(splitstatline) == len(liParameters):                     #check for consistency
                    liAddition=[]
                    for parameter in liParameters:                              #cycle through all parameters
                        liAddition.append(('%s = %s;\n'%                        #change setup.c to quasi fix all parameters
                            (self.diParameters[parameter]['variable'],          #to the result of the stat. analysis
                            splitstatline[self.diParameters[parameter]['number']])))
                    self.fnWriteConstraint2SetupC(liAddition)                   #write out
                    self.fnMake()                                               #compile changed setup.c
                ...
                #raw_input("Please enter ...")
                    ...
                    if bRecreateMolgroups==True:
                        subprocess.call(["./fit", "-o"])                        #write out mol.dat
                        self.fnLoadMolgroups()                                  #populate self.fndiMolgroups
...
def Auto(convergence=0.001):                                                    # automatic fit
    ReflPar=CReflPar()                                                          # prepare fit and clean directory
    ReflPar.fnMake()
    ...
    subprocess.call(['nice','./fit','-ezS','-n','21'],stdout=open(os.devnull,"w"))
    ...
    while 1:                                                                    # genetic runs until chisq<20 or not improving
        subprocess.call(['nice','./fit','-pezS','-n','51'],stdout=open(os.devnull,"w"))
...
def AutoFinish(convergence=0.001):
    ...
    while 1:                                                                    #Amoeba, approximate roughness
        subprocess.call(['nice','./fit','-peazS'],stdout=open(os.devnull,"w"))   #until chisq<10 or no improvement
...
def AutoFinish2(convergence=0.001):                                             # automatic fit, only local minimum refinement
    ...
    while 1:
        ...
        if (iGeneticIsUseless==1):                                              # skip genetic algorithm after it proved to be
            iGeneticSkipCount=iGeneticSkipCount+1                               # useless and give it a chance every 5 iterations
            print ' '
            print 'Genetic algorithm skipped'

        while ((iGeneticIsUseless==0) and (iGeneticSkipCount<6)):
            iGeneticIsUseless=0
            iGeneticSkipCount=0
            subprocess.call(['nice','./fit','-pez','-n','21'],stdout=open(os.devnull,"w"))
        ...
        while 1:
            subprocess.call(['nice','./fit','-peaz'],stdout=open(os.devnull,"w"))
        ...
        while 1:
            subprocess.call(['nice','./fit','-pelz'],stdout=open(os.devnull,"w"))
...
#-------------------------------------------------------------------------------
def fCalculateMolgroups(sMolgroup,fConfidence):
    ReflPar=CReflPar()
    ReflPar.fnCalculateMolgroupProperty(sMolgroup,fConfidence)
#-------------------------------------------------------------------------------
...
def DErr(convergence=0.001):                                                    #function has not yet been thoroughly tested
    ...
    ReflPar=CReflPar()                                                          #initialization
                        ...
                        if os.path.isfile(sMode+'Err.dat'):
                            ...
                            while 1:
                                try:
                                    ...
                                    iFailureCounter=iFailureCounter+1
                                    time.sleep(2)
...
def SErr(iiterations, convergence=0.01, sStatMode='is', fConfidence=0.9546):
    try:
        ReflPar=CReflPar()                                                      #fConfidence not yet used
        ReflPar.fnLoadAndPrintPar()
        ReflPar.fnBackup()
...
#-------------------------------------------------------------------------------
def StatAnalysis(fConfidence=0.9546):                                           # Analyzes MC output
    ReflPar=CReflPar()
    ReflPar.fnAnalyzeStatFile(fConfidence)

#-------------------------------------------------------------------------------
def StatTable(sTableName,fConfidence=0.9546):                                   # Produces Latex formatted table
    ReflPar=CReflPar()                                                          # using a template file
...
if __name__ == '__main__':
    if len(sys.argv)==1:
        ...
        if iContour==1:
            ...
            fCalculateMolgroups(sMolgroup,fConfidence)
        elif iEnv==1:
            if sStatMode=='none':                                               #Contour mode needs is based on a MC statistics
                sStatMode='is'                                                  #choose default
