...
def fnSet(sigma, bulknsld, startz, l_lipid, vf_bilayer):
    bilayer.Set(sigma, bulknsld, startz, l_lipid, l_lipid, vf_bilayer)

def fnWriteProfile(aArea, anSLD, dimension, stepsize, maxarea):
    return bilayer.WriteProfile(aArea, anSLD, dimension, stepsize, maxarea)
    
def fnGetCenter():
    return bilayer.GetCenter()

def modelformfactor(lq, l_lipid, vf_bilayer, sigma, bulknsld, prefactor):
    ...
    fnSet(sigma, bulknsld, startz, l_lipid, vf_bilayer)
    dd, aArea, anSL = fnWriteProfile(aArea, anSLD, dimension, stepsize, maxarea)
...
vc, nc = 0, 0
bilayer.Init(va1, na1, vm1, nm1, vh1,nh1, lh1, va2, na2, vm2, nm2, vh2, nh2, lh2, va3, na3, vm3, nm3, vh3, nh3,lh3, vc, nc)
...
M1 = Curve(modelformfactor, q_exp, form_exp, dform_exp, l_lipid=l_lipid, vf_bilayer=vf_bilayer, sigma=sigma, bulknsld=bulknsld, prefactor=prefactor)
M1.l_lipid.range(9,13)
M1.vf_bilayer.range(0.9,1.0)
M1.sigma.range(1.0, 4.0)
M1.bulknsld.range(9e-6,10e-6)
M1.prefactor.range(1500,2000)
