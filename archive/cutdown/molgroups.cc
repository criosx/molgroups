...
void tBLM::fnSetSigma(double dsigma)
{
    sigma=dsigma;
    substrate->sigma2=global_rough;
...
void tBLM_quaternary_chol::fnSet(double _sigma, double _global_rough, double _rho_substrate, double _nf_tether, double _mult_tether, double _l_tether, double _l_lipid1, double _l_lipid2, double _vf_bilayer, double _nf_lipid_2, double _nf_lipid_3, double _nf_chol, double _hc_substitution_1, double _hc_substitution_2){
...
DiscreteEuler::DiscreteEuler(double dstartposition, double dnormarea, double BetaStart, double BetaEnd, double BetaInc,
                             double GammaStart, double GammaEnd, double GammaInc, const char* strFileNameRoot, 
                             const char* strFileNameBeta, const char* strFileNameGamma, const char* strFileNameEnding)
{
    ...
    int i,j,k,itemp;
    FILE *fp;									
    char strFilename[200], buf[20], strTemp[40];
    ...
    j=0; dzold=0;
    //printf("File: %s.\n",strFilename);
    fscanf(fp, "%s %s %s %s", strTemp, strTemp, strTemp, strTemp);       //read out header
    while(!feof(fp)) {
        itemp=fscanf(fp, "%lf %lf %lf %lf", &dz, &darea, &dnSLProt, &dnSLDeut);
        if (itemp==4) {            
            //printf("%lf %lf %e %e \n", dz, darea, dnSLProt, dnSLDeut);
            if (j==1){
                dZSpacing=dz-dzold;
    ...
    for (dB=dBetaStart; dB<dBetaEnd; dB+=dBetaInc) {
        ...
        for (dG=dGammaStart; dG<dGammaEnd; dG+=dGammaInc) {
            ...
            fscanf(fp, "%s %s %s %s", strTemp, strTemp, strTemp, strTemp);       //read out header
            while(!feof(fp)) {
                itemp=fscanf(fp, "%lf %lf %lf %lf", &dz, &dnSLProt, &dnSLDeut, &darea);
                if (itemp==4) {
...
double DiscreteEuler::fnGetnSLD(double dz) {
    ...
    if ((iPosBinLow>=0) && (iPosBinHigh<=iNumberOfPoints) && (iBetaBinLow>=0) &&
        ...
        //printf("vol %g nsldprot %g nslddeut %g \n",dtemp4, dtemp1, dtemp2);
        if (dtemp4!=0) {
