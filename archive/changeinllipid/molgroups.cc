...
void nSLDObj::fnWriteGroup2File (FILE *fp, char *cName, int dimension, double stepsize){};
void nSLDObj::fnWriteData2File(FILE *fp, int dimension, double stepsize)
{
...
void   BoxErr::fnWriteGroup2File(FILE *fp, char *cName, int dimension, double stepsize)
{
        fprintf(fp, "BoxErr %s z %lf sigma %lf l %lf vol %lf nSL %e nf %lf \n",cName, z, sigma, l, vol, nSL, nf);
        nSLDObj::fnWriteData2File(fp, dimension, stepsize);
}
...
void   Box2Err::fnWriteGroup2File(FILE *fp, char *cName, int dimension, double stepsize)
{
...
void   Gaussian::fnWriteGroup2File(FILE *fp, char *cName, int dimension, double stepsize)
{
...
void   Parabolic::fnWriteGroup2File(FILE *fp, char *cName, int dimension, double stepsize)
{
...
void   StretchGaussian::fnWriteGroup2File(FILE *fp, char *cName, int dimension, double stepsize)
{
...
void PC::fnWriteGroup2File(FILE *fp, char *cName, int dimension, double stepsize)
{
...
void   PCm::fnWriteGroup2File(FILE *fp, char *cName, int dimension, double stepsize)
{
        //char *str = new char[80];
...
void FreeBox::fnWriteGroup2File(FILE *fp, char *cName, int dimension, double stepsize)
{
        //char *str = new char[80];
...
void tBLM::fnAdjustParameters(){
  ...
  //outer hydrocarbons
  l_ohc=l_lipid2*(1-volmethyllipid/volacyllipid);
  nf_ohc_lipid=1;
  ...
  //inner hydrocarbons
  l_ihc=l_lipid1*(1-volmethyllipid/volacyllipid);
  ...
  //outer PC headgroup
  headgroup2->nf=c_s_ohc*c_A_ohc*nf_ohc_lipid;
  
  //inner PC headgroup
  headgroup1->nf=c_s_ihc*c_A_ihc*nf_ihc_lipid;
  ...
  tetherg->l=tetherg->vol/((volacyltether-volmethyltether)/lipid1->l);
  tetherg->nf=c_s_tg*c_A_tg*c_V_tg;
  
  //tether EO part
  l_EO=l_tether*(1-tetherg->vol/tether->vol);
  V_EO=tether->vol;
...
void tBLM::fnWriteGroup2File(FILE *fp, char *cName, int dimension, double stepsize)
{
        //char *str = new char[80];
...
void ssBLM::fnWriteGroup2File(FILE *fp, char *cName, int dimension, double stepsize)
{
        //char *str = new char[80];
...
void tBLM_binary::fnAdjustParameters(){
  ...
  //outer headgroups
  headgroup2->nf=c_s_ohc*c_A_ohc*nf_ohc_lipid;
  headgroup2_2->nf=c_s_ohc*c_A_ohc*nf_ohc_lipid_2;
  
  //inner headgroups
  headgroup1->nf=c_s_ihc*c_A_ihc*nf_ihc_lipid;
  headgroup1_2->nf=c_s_ihc*c_A_ihc*nf_ihc_lipid_2;
...
void tBLM_binary::fnWriteGroup2File(FILE *fp, char *cName, int dimension, double stepsize)
{
        //char *str = new char[80];
...
tBLM_HC18_DOPC_DOPS::tBLM_HC18_DOPC_DOPS()
{
	...
	headgroup1_2->vol=244;                 //estimate               
	headgroup2_2->vol=244;                
	headgroup1_2->nSL=8.4513e-4;          
	headgroup2_2->nSL=8.4513e-4;          
	headgroup1_2->l=7;						//estimate
	headgroup2_2->l=7;
	
    volacyllipid_2=968;
    nslacyllipid_2=-3.7506e-4;
    volmethyllipid_2=98.8;
    nslmethyllipid_2=-9.15e-5;
...
//------------------------------------------------------------------------------------------------------
//writes a constant in the style of molgroups to file
...
//------------------------------------------------------------------------------------------------------
//fills aArea and anSL with zeros
